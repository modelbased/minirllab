...
class Actor(nn.Module):
    def __init__(self, obs_dim, action_dim, hidden_dim=256):
        ...
        # HACK: Manual scaling to environment. Correct scaling can affect learning speed
        self.action_scale = 0.4 # Humanoid-v4 is 0.4
...
class Agent:
    def __init__(self, 
        ...
        # Make global
        self.name           = "sac_v01_l2droq"          # name for logging
        ...
        "l2init_lambda_q"   : l2init_lambda_q,
        "l2init_lambda_a"   : l2init_lambda_a,
        ...
        self.actor       = Actor(obs_dim, action_dim, self.h.a_hidden_dim).to(device)
        ...
        # Storage setup
        self.rb          = ReplayBuffer(self.obs_dim, self.action_dim, buffer_size, num_env, device=self.device)
        self.global_step = 0

        # L2 Init for each model
        self.l2_qf1     = L2NormRegularizer(self.qf1,   lambda_reg=l2init_lambda_q, device=self.device)
        self.l2_qf2     = L2NormRegularizer(self.qf2,   lambda_reg=l2init_lambda_q, device=self.device)
        self.l2_actor   = L2NormRegularizer(self.actor, lambda_reg=l2init_lambda_a, device=self.device)
    ...
    def update(self):
        ...
        if self.global_step > self.h.learn_start:
            ...
            for replay in range(0,self.h.replay_ratio):
                ...
                nn.utils.clip_grad_norm_(self.qf1.parameters(), self.h.q_max_grad_norm)
            ...
            if self.global_step % self.h.policy_freq == 0:  # TD 3 Delayed update support
                for _ in range(self.h.policy_freq):  # compensate for the delay by doing 'actor_update_interval' instead of 1
                    ...
                    nn.utils.clip_grad_norm_(list(self.actor.parameters()), self.h.a_max_grad_norm)
                    ...
                    nn.utils.clip_grad_norm_([self.log_alpha], self.h.a_max_grad_norm)
                    self.alpha_optim.step()
                    # self.alpha = self.log_alpha.exp().item()
                    self.alpha = self.log_alpha.exp().detach().clone() #TODO: Test this does not break learning
