...
'''
    ...
    Baseline2 with the following chnages
    - L2 Init regularisation from https://arxiv.org/abs/2308.11958v1 
'''
...
class L2NormRegularizer:
    ...
    def loss(self, model):
        ...
        for name, param in model.named_parameters():
            ...
            l2_loss += torch.norm(param - initial_param)**2
...
class Actor(nn.Module):
    ...
    def forward(self, obs, action=None):
        ...
        x = x + self.r0[0] * self.nonlin(self.fc1(x))
...
class Critic(nn.Module):
    def __init__(self, obs_dim, hidden_dim=64, critic_mult=4):
        super(Critic, self).__init__()

        # Critic network (C47, independent critic performs better https://arxiv.org/abs/2006.05990)
        self.fc0 = init_weights(nn.Linear(obs_dim, hidden_dim * critic_mult)) # wider critic preferred (https://arxiv.org/abs/2006.05990)
        self.fc1 = init_weights(nn.Linear(hidden_dim * critic_mult, hidden_dim)) 
        self.out = init_weights(nn.Linear(hidden_dim, 1), std=1.0) # Last layer init near one (C57, https://arxiv.org/abs/2006.05990)
        self.nonlin = nn.Tanh()

        # ReZero for deep networks https://arxiv.org/abs/2003.04887
        # self.r0 = nn.Parameter(torch.zeros(6)) # needs constant hidden dim
...
class Agent:
    def __init__(self, obs_dim, action_dim, run_steps, num_env=1, device='cpu', seed=42):
        ...
        # Make global
        self.name           = "ppo_v01_l2_init"       # name for logging
        ...
        self.actor   = (Actor(self.obs_dim, self.action_dim, hidden_dim=64, rpo_alpha=self.h.rpo_alpha, device=device).to(self.device))
        self.optim_a = torch.optim.AdamW(self.actor.parameters(), lr=self.h.adam_lr, eps=self.h.adam_eps, betas=self.h.adam_betas, weight_decay=self.h.weight_decay)
        
        self.critic  = (Critic(self.obs_dim, hidden_dim=64, critic_mult=4).to(self.device))
        self.optim_c = torch.optim.AdamW(self.critic.parameters(), lr=self.h.adam_lr, eps=self.h.adam_eps, betas=self.h.adam_betas, weight_decay=self.h.weight_decay)

        # L2 Init for each model
        self.l2_actor  = L2NormRegularizer(self.actor, lambda_reg=0.01)
        self.l2_critic = L2NormRegularizer(self.critic, lambda_reg=0.01)
        ...
        if self.global_step % self.h.update_step == 0 and self.global_step != 0:
            ...
            for epoch in range(self.h.update_epochs):
                ...
                for start in range(0, batch_end, self.h.mb_size):
                    ...
                    mb_newvalues = self.critic(mb_obs)
                    ...
                    l2_loss_actor = self.l2_actor.loss(self.actor)
                    self.p_loss = torch.max(p_loss1, p_loss2).mean() - self.h.ent_coef * self.entropy_loss + l2_loss_actor

                    # Value loss
                    mb_newvalues = mb_newvalues.view(-1)
                    l2_loss_critic = self.l2_critic.loss(self.critic)
                    self.v_loss = 0.5 * ((mb_newvalues - b_returns[mb_inds]) ** 2).mean() + l2_loss_critic
