...
class Agent:
    def __init__(self, env_spec, buffer_size, num_env=1, device='cpu', seed=42):
        ...
        # Make global
        self.name           = "ppo_baseline"         # name for logging
        ...
        self.chronos_total      = 0.0     # time taken for training update (use torch.cuda.Event() for more accurate gpu timing)
    ...
    def update(self):
        ...
        if self.global_step % self.h.update_step == 0 and self.global_step != 0:
            ...
            batch_end = self.rb.ptr - 1 # index to last element
            ...
            self.rb.flush()
            self.chronos_total = time.time() - chronos_start

        return updated, self.chronos_total
    ...
    def save(self, path='./checkpoints/'):
        ...
        models['Critic'] = self.critic.state_dict()
    ...
    def load(self, path='./checkpoints/'):
        ...
        self.critic.load_state_dict(models_file['Critic'])
